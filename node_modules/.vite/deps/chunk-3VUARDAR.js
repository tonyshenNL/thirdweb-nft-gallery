  ...
  "node_modules/@account-abstraction/sdk/dist/src/DeterministicDeployer.js"(exports) {
    ...
    var DeterministicDeployer = class _DeterministicDeployer {
      static getAddress(ctrCode, salt = 0, params = []) {
        return _DeterministicDeployer.getDeterministicDeployAddress(ctrCode, salt, params);
      }
      static async deploy(ctrCode, salt = 0, params = []) {
        return await _DeterministicDeployer.instance.deterministicDeploy(ctrCode, salt, params);
      }
      ...
      async isDeployerDeployed() {
        return await this.isContractDeployed(_DeterministicDeployer.proxyAddress);
      }
      async deployFactory() {
        if (await this.isContractDeployed(_DeterministicDeployer.proxyAddress)) {
          return;
        }
        const bal = await this.provider.getBalance(_DeterministicDeployer.deploymentSignerAddress);
        const neededBalance = ethers_1.BigNumber.from(_DeterministicDeployer.deploymentGasLimit).mul(_DeterministicDeployer.deploymentGasPrice);
        const signer = this.provider.getSigner();
        if (bal.lt(neededBalance)) {
          await signer.sendTransaction({
            to: _DeterministicDeployer.deploymentSignerAddress,
            value: neededBalance,
            gasLimit: _DeterministicDeployer.deploymentGasLimit
          });
        }
        await this.provider.send("eth_sendRawTransaction", [_DeterministicDeployer.deploymentTransaction]);
        if (!await this.isContractDeployed(_DeterministicDeployer.proxyAddress)) {
      ...
      async getDeployTransaction(ctrCode, salt = 0, params = []) {
        ...
        const ctrEncoded = _DeterministicDeployer.getCtrCode(ctrCode, params);
        return {
          to: _DeterministicDeployer.proxyAddress,
      ...
      static getDeterministicDeployAddress(ctrCode, salt = 0, params = []) {
        const saltEncoded = (0, utils_1.hexZeroPad)((0, utils_1.hexlify)(salt), 32);
        const ctrCode1 = _DeterministicDeployer.getCtrCode(ctrCode, params);
        return "0x" + (0, utils_1.keccak256)((0, utils_1.hexConcat)([
          "0xff",
          _DeterministicDeployer.proxyAddress,
          saltEncoded,
          (0, utils_1.keccak256)(ctrCode1)
        ])).slice(-40);
      }
      async deterministicDeploy(ctrCode, salt = 0, params = []) {
        const addr = _DeterministicDeployer.getDeterministicDeployAddress(ctrCode, salt, params);
      ...
      static init(provider) {
        this._instance = new _DeterministicDeployer(provider);
      }
      static get instance() {
...
//# sourceMappingURL=chunk-3VUARDAR.js.map
