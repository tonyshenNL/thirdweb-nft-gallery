...
var contractPublisher = require('./contract-publisher-cf47fb57.cjs.prod.js');
...
async function getEncodedConstructorParamsForThirdwebContract(contractName, chainId, storage) {
  ...
  if (extendedMetadata?.routerType === "plugin" || extendedMetadata?.routerType === "dynamic") {
    const deploymentInfo = await contractPublisher.getDeploymentInfo(publishUri, storage, provider, create2Factory, clientId, secretKey);
...
class UserWallet {
  ...
  async transfer(to, amount) {
    ...
    const [resolvedTo, resolvedCurrency, amountInWei] = await Promise.all([contractPublisher.resolveAddress(to), contractPublisher.resolveAddress(currencyAddress), contractPublisher.normalizePriceValue(this.connection.getProvider(), amount, currencyAddress)]);
...
class ContractRegistry extends contractPublisher.ContractWrapper {
  ...
  addContracts = /* @__PURE__ */contractPublisher.buildTransactionFunction(async contractAddresses => {
    ...
    const encoded = (await Promise.all(contractAddresses.map(addr => contractPublisher.resolveAddress(addr)))).map(address => contractEncoder.encode("add", [deployerAddress, address]));
    return contractPublisher.Transaction.fromContractWrapper({
      contractWrapper: this,
      method: "multicall",
      args: [encoded]
  ...
  removeContracts = /* @__PURE__ */contractPublisher.buildTransactionFunction(async contractAddresses => {
    const deployerAddress = await this.getSignerAddress();
    const contractEncoder = new contractPublisher.ContractEncoder(this);
    const encoded = (await Promise.all(contractAddresses.map(addr => contractPublisher.resolveAddress(addr)))).map(address => contractEncoder.encode("remove", [deployerAddress, address]));
    return contractPublisher.Transaction.fromContractWrapper({
      contractWrapper: this,
      method: "multicall",
      args: [encoded]
...
class ContractDeployer extends contractPublisher.RPCConnectionHandler {
  ...
  deployViaAutoFactory = /* @__PURE__ */contractPublisher.buildDeployTransactionFunction(async (publishMetadataUri, deployMetadata, signer, initializerFunction, paramValues, options) => {
    ...
    const deploymentInfo = await contractPublisher.getDeploymentInfo(publishMetadataUri, this.storage, this.getProvider(), create2Factory, this.options.clientId, this.options.secretKey);
    ...
    await contractPublisher.deployWithThrowawayDeployer(signer, transactionsForThrowawayDeployer, options);

    // send each transaction directly to Create2 factory
    // process txns one at a time
    for (const tx of transactionsforDirectDeploy) {
      try {
        await contractPublisher.deployContractDeterministic(signer, tx, options);
      } catch (e) {
  ...
  async getTransactionsForDeploy(publishMetadataUri, options) {
    ...
    if (extendedMetadata && extendedMetadata.factoryDeploymentData && (extendedMetadata.isDeployableViaProxy || extendedMetadata.isDeployableViaFactory || extendedMetadata.deployType === "autoFactory") && !forceDirectDeploy) {
      ...
      if (!implementationAddress || extendedMetadata.deployType === "autoFactory") {
        ...
        const deploymentInfo = await contractPublisher.getDeploymentInfo(publishMetadataUri, this.storage, this.getProvider(), create2FactoryAddress, this.options.clientId, this.options.secretKey);
        const transactionsToSend = deploymentInfo.filter(i => i.transaction.data && i.transaction.data.length > 0);
        const transactionsforDirectDeploy = transactionsToSend.filter(i => {
          return i.type !== "infra";
        }).map(i => i.transaction);
        ...
        const transactionBatches = contractPublisher.createTransactionBatches(transactionsForThrowawayDeployer);
